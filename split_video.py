...
#     if not os.path.isdir('./data/encode_downscale/'+ sequence):
#         os.mkdir('./data/encode_downscale/'+ sequence)
...
for x, videoname in enumerate(video_list):
    ...
    for i in range(num_sequence):
        path = f'./data/encode_downscale_video/{videoname}_{i}'
        if not os.path.isdir(path):
            os.mkdir(path)

        for QP in QP_List:
            #encode video
            command_encode = f'E:/codec/vvc_codec/x265.exe --input E:/codec/data/{videoname}.yuv --fps {fr[x]} --seek {start}  --frames 32 --input-res 312x180 --input-depth 8 --psnr --qp {QP} --input-csp i420 --output  E:/codec/data/encode_downscale_video/{videoname}_{i}/{videoname}_QP{QP}.bin --csv  E:/codec/data/encode_downscale_video/{videoname}_{i}/{videoname}_QP{QP}.csv --csv-log-level 4 >>  E:/codec/data/encode_downscale_video/{videoname}_{i}/{videoname}_QP{QP}.txt'
            os.system(command_encode)

            #decode video
            command_decode = f'E:/codec/vvc_codec/TAppDecoder.exe -b E:/codec/data/encode_downscale_video/{videoname}_{i}/{videoname}_QP{QP}.bin -o E:/codec/data/encode_downscale_video/{videoname}_{i}/{videoname}_QP{QP}.yuv'
            os.system(command_decode)
        start += 32
